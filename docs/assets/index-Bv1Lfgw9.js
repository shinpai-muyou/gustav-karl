(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))e(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const c of i.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&e(c)}).observe(document,{childList:!0,subtree:!0});function t(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function e(a){if(a.ep)return;a.ep=!0;const i=t(a);fetch(a.href,i)}})();const w=`//#version 450\r
//#pragma shader_stage(fragment)\r
//#extension GL_EXT_samplerless_texture_functions : enable\r
/*\r
================================================================================\r
克尔-纽曼黑洞 (Kerr-Newman Black Hole) 实时广义相对论渲染器\r
Kerr-Newman Black Hole Real-time General Relativity Renderer\r
================================================================================\r
\r
[ 简介 / Introduction ]\r
    本Shader实现了基于物理的克尔-纽曼时空（带电旋转黑洞）光线追踪。\r
    This shader implements physically based ray-tracing in Kerr-Newman spacetime\r
    (charged rotating black hole).\r
\r
    实现了完整的时空拓扑：外视界、内视界、内外能层、奇环，以及反宇宙。\r
    相机可在全空间内自由移动。\r
    Implements complete spacetime topology: outer/inner event horizons,\r
    outer/inner ergospheres, ringularity, and the antiverse.\r
    The camera can move freely throughout the space.\r
\r
    支持裸奇点。\r
    Supports naked singularities.\r
\r
    支持切换静态观者（Static）与自由落体观者（Free-falling/Raindrop）。\r
    Supports switching between Static observers and Free-falling (Raindrop) observers.\r
\r
    完全拟真广义相对论效应，包含吸积盘、相对论喷流、引力透镜、\r
    多普勒频移与引力红移。\r
    Fully simulates GR effects: accretion disk, relativistic jets, gravitational\r
    lensing, Doppler shift, and gravitational redshift.\r
\r
--------------------------------------------------------------------------------\r
\r
[ 操作方式 / Controls ]\r
 键盘控制移动，鼠标控制视角：\r
 Keyboard controls movement, mouse controls camera angle:\r
\r
     [W] / [S] : 前进 / 后退 (Forward / Backward)\r
     [A] / [D] : 向左 / 向右 (Left / Right)\r
     [R] / [F] : 上升 / 下降 (Up / Down - Relative to View)\r
...
开启网格(iGrid=1，2)会禁用视界附近的提前剔除优化，导致帧率下降。\r
Enabling the grid (iGrid=1，2) disables early-culling optimizations\r
near the horizon, causing a drop in frame rate.\r
\r
吸积盘体积云噪声计算开销较大。\r
Volumetric cloud noise calculations for the accretion disk are expensive.\r
\r
极端黑洞或裸奇点 (a^2+Q^2 >≈ M^2) 需要消耗大量步数，也会导致帧率下降。\r
Extreme black holes or naked singularities (a^2+Q^2 >≈ M^2)\r
require many steps, which will also lower the frame rate.\r
--------------------------------------------------------------------------------\r
\r
[ 已知问题 / Known Issues ]\r
这个项目尚未完全完成。\r
This project is a work in progress and is not yet fully finished.\r
\r
目前的吸积盘和喷流内部子采样逻辑仍存在未定位的Bug，会导致层纹伪影。\r
There is currently an unidentified bug in the internal sub-sampling logic\r
for the accretion disk and jets, causing banding artifacts.\r
\r
相机在落体观者模式下，位于奇环附近时可能出现数值不稳定。\r
Numerical instability may occur near the ring singularity\r
when using the free-falling observer mode.\r
\r
================================================================================\r
\r
Code introduction and development tutorial（代码介绍与开发教程）:\r
...
#define textureQueryLod(s, d) vec2(0.0)\r
\r
// -----------------------------------------------------------------------------\r
// 物理与渲染参数 (对应原 BlackHoleArgs Uniform)\r
// 你可以在这里调整数值\r
// -----------------------------------------------------------------------------\r
#define iFovRadians  60.0 * 0.01745329\r
#define iGrid           0      // 0:关, 1:黑体 2:固定色\r
#define iObserverMode   0      // 0:静态观者, 1:落体观者\r
\r
#define iBlackHoleTime          (2.0*iTime)  // 使用 Shadertoy 时间\r
#define iBlackHoleMassSol       (1e7)     // 太阳质量倍数\r
#define iSpin                   0.99      // 自旋 必须与BufferB中iSpin一致！It must be modified synchronously with iSpin in BufferB！\r
#define iQ                      0.0      // 电荷\r
\r
#define iMu                     1.0      // 吸积物质比荷\r
#define iAccretionRate          (5e-4)      // 吸积率\r
\r
#define iInterRadiusRs          1.5      // 吸积盘内半径\r
#define iOuterRadiusRs          25.0     // 吸积盘外半径\r
#define iThinRs                 0.75     // 吸积盘半厚度\r
#define iHopper                 0.24      // 厚度斜率\r
#define iBrightmut              1.0      // 亮度\r
#define iDarkmut                0.5     // 不透明度\r
#define iReddening              0.3      // 红化\r
#define iSaturation             0.5      // 饱和度\r
#define iBlackbodyIntensityExponent 0.5\r
#define iRedShiftColorExponent      3.0\r
#define iRedShiftIntensityExponent  4.0\r
\r
#define iPhotonRingBoost             7.0 //光子环亮度增亮\r
#define iPhotonRingColorTempBoost    2.0 //光子环颜色增蓝\r
#define iBoostRot                    0.75 //增强在自旋非0的非对称程度\r
\r
...
\r
#define ENABLE_HEAT_HAZE        1       // 1 = 开启热浪折射\r
#define HAZE_STRENGTH           0.03    // 折射强度\r
#define HAZE_SCALE              5.2     // 噪声频率\r
#define HAZE_DENSITY_THRESHOLD  0.1     // 密度阈值\r
#define HAZE_LAYER_THICKNESS    0.8     // 厚度范围倍数\r
#define HAZE_RADIAL_EXPAND      0.8     // 径向范围倍数\r
#define HAZE_ROT_SPEED          0.2     // 盘热气旋转速度系数 (相对于开普勒速度)\r
#define HAZE_FLOW_SPEED         0.15     // 喷流速度系数\r
#define HAZE_PROBE_STEPS        12      // 试探步数\r
#define HAZE_STEP_SIZE          0.06    // 每步长度 (Rg)\r
#define HAZE_DEBUG_MASK         0       // 1 = 显示热气遮罩 Debug\r
#define HAZE_DEBUG_VECTOR       0       // 1 = 显示力场向量 Debug\r
#define HAZE_DISK_DENSITY_REF   (iBrightmut * 30.0) \r
#define HAZE_JET_DENSITY_REF    (iJetBrightmut * 1.0)\r
\r
#define iBlendWeight            0.5      //TAA前后帧混合权重\r
\r
\r
\r
...
\r
float PerlinNoise(vec3 Position)//以后改成读lut\r
{\r
...
{\r
    ...
    uvec4 rz = uvec4(h, h*16807U, h*48271U, h*69621U); //see: http://random.mat.sbg.ac.at/results/karl/server/node4.html\r
    return vec4((rz >> 1) & uvec4(0x7fffffffU))/float(0x7fffffff);\r
}\r
\r
\r
vec3 stars(vec3 p)//from  https://www.shadertoy.com/view/fl2Bzd\r
{\r
    vec3 col = vec3(0);\r
    ...
    {\r
        ...
        vec3 pp = fp - ((rand.xyz-0.5)*.6 + 0.5)*pl; //don't displace points away from the cube faces\r
        float pr = length(ip) - rad;   \r
        if (rand.w > (dens - dens*pr*0.035)) pp += 1e6;\r
\r
        float d = dot(pp, pp);\r
        d /= pow(fract(rand.w*172.1), 32.) + .25;\r
        float bri = dot(rand.xyz*(1.-pl),vec3(1)); //since one random value is unused to displace, we can reuse\r
...
//from https://www.shadertoy.com/view/4t3BWl\r
const int ITERATIONS = 40;   //use less value if you need more performance\r
...
const float ZCELL_SIZE = 1. * (STRIP_CHAR_HEIGHT * STRIP_CHARS_MAX);  //the multiplier can't be less than 1.\r
const float XYCELL_SIZE = 12. * STRIP_CHAR_WIDTH;  //the multiplier can't be less than 1.\r
\r
const int BLOCK_SIZE = 10;  //in cells\r
const int BLOCK_GAP = 2;    //in cells\r
\r
...
\r
//        ----  symbols  ----\r
//  Slightly modified version of "runes" by FabriceNeyret2 -  https://www.shadertoy.com/view/4ltyDM\r
//  Which is based on "runes" by otaviogood -  https://shadertoy.com/view/MsXSRn\r
\r
float rune_line(vec2 p, vec2 a, vec2 b) {   // from https://www.shadertoy.com/view/4dcfW8\r
    p -= a, b -= a;\r
	float h = clamp(dot(p, b) / dot(b, b), 0., 1.);   // proj coord on line\r
	return length(p - b * h);                         // dist to segment\r
}\r
\r
float rune(vec2 U, vec2 seed, float highlight)\r
{\r
	float d = 1e5;\r
	for (int i = 0; i < 4; i++)	// number of strokes\r
	{\r
        vec4 pos = hash4(seed);\r
		seed += 1.;\r
\r
		// each rune touches the edge of its box on all 4 sides\r
		if (i == 0) pos.y = .0;\r
		if (i == 1) pos.x = .999;\r
		if (i == 2) pos.x = .0;\r
		if (i == 3) pos.y = .999;\r
		// snap the random line endpoints to a grid 2x3\r
		vec4 snaps = vec4(2, 3, 2, 3);\r
		pos = ( floor(pos * snaps) + .5) / snaps;\r
\r
		if (pos.xy != pos.zw)  //filter out single points (when start and end are the same)\r
		    d = min(d, rune_line(U, pos.xy, pos.zw + .001) ); // closest line\r
	}\r
...
//        ----  digital rain  ----\r
\r
// xy - horizontal, z - vertical\r
vec3 rain(vec3 ro3, vec3 rd3, float time) {\r
    vec4 result = vec4(0.);\r
\r
    // normalized 2d projection\r
    vec2 ro2 = vec2(ro3);\r
    vec2 rd2 = normalize(vec2(rd3));\r
\r
    // we use formulas \`ro3 + rd3 * t3\` and \`ro2 + rd2 * t2\`, \`t3_to_t2\` is a multiplier to convert t3 to t2\r
    bool prefer_dx = abs(rd2.x) > abs(rd2.y);\r
    float t3_to_t2 = prefer_dx ? rd3.x / rd2.x : rd3.y / rd2.y;\r
\r
    // at first, horizontal space (xy) is divided into cells (which are columns in 3D)\r
    // then each xy-cell is divided into vertical cells (along z) - each of these cells contains one raindrop\r
\r
    ivec3 cell_side = ivec3(step(0., rd3));      //for positive rd.x use cell side with higher x (1) as the next side, for negative - with lower x (0), the same for y and z\r
    ivec3 cell_shift = ivec3(sign(rd3));         //shift to move to the next cell\r
\r
    //  move through xy-cells in the ray direction\r
    float t2 = 0.;  // the ray formula is: ro2 + rd2 * t2, where t2 is positive as the ray has a direction.\r
    ivec2 next_cell = ivec2(floor(ro2/XYCELL_SIZE));  //first cell index where ray origin is located\r
    for (int i=0; i<ITERATIONS; i++) {\r
        ivec2 cell = next_cell;  //save cell value before changing\r
        float t2s = t2;          //and t\r
\r
        //  find the intersection with the nearest side of the current xy-cell (since we know the direction, we only need to check one vertical side and one horizontal side)\r
        vec2 side = vec2(next_cell + cell_side.xy) * XYCELL_SIZE;  //side.x is x coord of the y-axis side, side.y - y of the x-axis side\r
        vec2 t2_side = (side - ro2) / rd2;  // t2_side.x and t2_side.y are two candidates for the next value of t2, we need the nearest\r
        if (t2_side.x < t2_side.y) {\r
            t2 = t2_side.x;\r
            next_cell.x += cell_shift.x;  //cross through the y-axis side\r
        } else {\r
            t2 = t2_side.y;\r
            next_cell.y += cell_shift.y;  //cross through the x-axis side\r
        }\r
        //now t2 is the value of the end point in the current cell (and the same point is the start value in the next cell)\r
\r
        //  gap cells\r
...
\r
        //  return to 3d - we have start and end points of the ray segment inside the column (t3s and t3e)\r
        float t3s = t2s / t3_to_t2;\r
\r
        //  move through z-cells of the current column in the ray direction (don't need much to check, two nearest cells are enough)\r
        float pos_z = ro3.z + rd3.z * t3s;\r
        float xycell_hash = hash(vec2(cell));\r
        float z_shift = xycell_hash*11. - time * (0.5 + xycell_hash * 1.0 + xycell_hash * xycell_hash * 1.0 + pow(xycell_hash, 16.) * 3.0);  //a different z shift for each xy column\r
        float char_z_shift = floor(z_shift / STRIP_CHAR_HEIGHT);\r
        z_shift = char_z_shift * STRIP_CHAR_HEIGHT;\r
        int zcell = int(floor((pos_z - z_shift)/ZCELL_SIZE));  //z-cell index\r
        for (int j=0; j<2; j++) {  //2 iterations is enough if camera doesn't look much up or down\r
            //  calcaulate coordinates of the target (raindrop)\r
...
\r
            //  We have a line segment (t0,t). Now calculate the distance between line segment and cell target (it's easier in 2d)\r
            vec2 s = target - ro2;\r
            float tmin = dot(s, rd2);  //tmin - point with minimal distance to target\r
            if (tmin >= t2s && tmin <= t2) {\r
                float u = s.x * rd2.y - s.y * rd2.x;  //horizontal coord in the matrix strip\r
                if (abs(u) < target_rad) {\r
                    u = (u/target_rad + 1.) / 2.;\r
                    float z = ro3.z + rd3.z * tmin/t3_to_t2;\r
                    float v = (z - target_z) / target_length;  //vertical coord in the matrix strip\r
                    if (v >= 0.0 && v < 1.0) {\r
                        float c = floor(v * chars_count);  //symbol index relative to the start of the strip, with addition of char_z_shift it becomes an index relative to the whole cell\r
                        float q = fract(v * chars_count);\r
                        vec2 char_hash = hash2(vec2(c+char_z_shift, cell_hash2.x));\r
                        if (char_hash.x >= 0.1 || c == 0.) {  //10% of missed symbols\r
                            float time_factor = floor(c == 0. ? time*5.0 :  //first symbol is changed fast\r
                                    time*(1.0*cell_hash2.z +   //strips are changed sometime with different speed\r
                                            cell_hash2.w*cell_hash2.w*4.*pow(char_hash.y, 4.)));  //some symbols in some strips are changed relatively often\r
                            float a = random_char(vec2(char_hash.x, time_factor), vec2(u,q), max(1., 3. - c/2.)*0.2);  //alpha\r
                            a *= clamp((chars_count - 0.5 - c) / 2., 0., 1.);  //tail fade\r
                            if (a > 0.) {\r
            ...
            // not found in this cell - go to next vertical cell\r
            zcell += cell_shift.z;\r
        }\r
        // go to next horizontal cell\r
    }\r
\r
...
{\r
    ...
    if (Status > 1.5) { // Antiverse (Status == 2.0)\r
...
// =============================================================================\r
// SECTION 4: 广相计算。Y为自旋方向，ingoing方向笛卡尔形式kerrschild系。+++-。\r
// =============================================================================\r
\r
const float CONST_M = 0.5; // [PHYS] Mass M = 0.5,DONT CHANGE THIS\r
const float EPSILON = 1e-6;\r
\r
// [TENSOR] Flat Space Metric eta_uv = diag(1, 1, 1, -1)\r
...
\r
//PhysicalSpinA和PhysicalQ是有量纲量（无量纲量乘M，即乘0.5）。\r
\r
...
\r
    ...
    float rho2 = dot(p.xz, p.xz); // x^2 + z^2\r
    float y2 = p.y * p.y;\r
    \r
...
float GetZamoOmega(float r, float a, float Q, float y) {\r
    ...
    \r
    // Delta = r^2 - 2Mr + a^2 + Q^2 (M=0.5)\r
    float Delta = r2 - r + a2 + Q * Q;\r
    \r
    // Sigma = r^2 + a^2 cos^2 theta\r
    float Sigma = r2 + a2 * cos2;\r
    \r
    // metric term A = (r^2+a^2)^2 - Delta * a^2 * sin^2 theta\r
    float A_metric = (r2 + a2) * (r2 + a2) - Delta * a2 * sin2;\r
    \r
    // Omega_ZAMO = 2Mra / A (for Q=0), with Q: a(2Mr - Q^2) / A\r
    // 2Mr = r (since M=0.5, 2M=1.0) -> r\r
    return a * (r - Q * Q) / max(1e-9, A_metric);\r
}\r
\r
// 求解射线与 Kerr-Schild 常数 r 椭球面的交点\r
// 方程: (x^2 + z^2)/(r^2 + a^2) + y^2/r^2 = 1\r
// 返回 vec2(t1, t2)，如果没有交点返回 vec2(-1.0)\r
vec2 IntersectKerrEllipsoid(vec3 O, vec3 D, float r, float a) {\r
    float r2 = r * r;\r
    float a2 = a * a;\r
    float R_eq_sq = r2 + a2; // 赤道半径平方\r
    float R_pol_sq = r2;     // 极半径平方\r
    \r
    // 椭球方程: B(x^2 + z^2) + A(y^2) = A*B\r
    // 其中 A = R_eq_sq, B = R_pol_sq\r
    float A = R_eq_sq;\r
    float B = R_pol_sq;\r
    \r
    // 代入射线 P = O + D*t\r
    // (B*Dx^2 + B*Dz^2 + A*Dy^2) t^2 + ...\r
    ...
    \r
    if (abs(qa) < 1e-9) return vec2(-1.0); // 线性退化，忽略\r
    \r
...
struct KerrGeometry {\r
    ...
    vec3  grad_r;         \r
    vec3  grad_f;         \r
    vec4  l_up;           // l^u = (lx, ly, lz, -1)\r
    vec4  l_down;         // l_u = (lx, ly, lz, 1)\r
    float inv_r2_a2;\r
    float inv_den_f;      \r
    float num_f;          \r
};\r
\r
//fade用于在接近包围盒边界时强行过渡为平直时空。直接乘在f上。下文中gravityfade同。\r
\r
...
\r
//ks系的形式使得度规与矢量的乘法可以优化\r
//升指标和降指标。虽然变量名用的P，但是可以用于任何符合变换规则的矢量\r
//  P^u = g^uv P_v\r
// g^uv = eta^uv - f * l^u * l^v\r
vec4 RaiseIndex(vec4 P_cov, KerrGeometry geo) {\r
    // eta^uv = diag(1, 1, 1, -1)\r
...
\r
    ...
    if (ObserverMode == 1) {\r
        ...
        U_up = mix(U_up,vec4(U_spatial, Ut),GravityFade);//在包围盒边界回退到静态观者\r
\r
    }\r
       \r
...
\r
    ...
    //副轴，环向或X。注意这个系只是中转，在极点强取方向不会改变视线朝向，只会改变畸变方向，而两极处在平行赤道面上正好没有畸变\r
...
\r
            ...
            \r
            P.xyz *= mix(k,1.0,clamp(abs(k-1.0)/0.1-1.0,0.0,1.0));//修正过强时强行修正会炸，在0.9到0.8过渡回退到不修正\r
        }\r
    }\r
}\r
//通过缩放动量空间部分修正哈密顿量\r
...
State GetDerivativesAnalytic(State S, float PhysicalSpinA, float PhysicalQ, float fade, inout KerrGeometry geo) {\r
    ...
    \r
    // Force = 0.5 * [ grad_f * (l.P)^2 + 2f(l.P) * grad(l.P) ]\r
...
\r
//检测试探步是否穿过奇环面。Rk4里小步的符号需要实时更新不然会被弹飞\r
...
\r
    // k1 Step\r
    // State k1 = GetDerivativesAnalytic(s0, PhysicalSpinA, PhysicalQ, fade, geo0);\r
\r
    // k2 Step\r
    State s1; \r
...
\r
    ...
    if(finalSign>0.0){//antiverse侧修正有可能造成数值问题，暂时关停\r
...
// =============================================================================\r
// SECTION 6: 吸积盘与喷流,经纬网\r
// =============================================================================\r
\r
// heat haze热浪折射，噪声准备\r
...
{\r
    ...
    \r
    // 给采样坐标加一个任意角度的旋转矩阵，防止噪声晶格与吸积盘平面(XZ)对齐\r
...
{\r
    ...
    \r
    // 喷流不应在吸积盘内部太强 (y 接近 0 时)，虽然 JetColor 也有处理，\r
    // 这里加一个平滑淡入防止在盘中心产生奇怪的重叠\r
...
bool IsInHazeBoundingVolume(vec3 pos, float probeDist, float OuterRadius) {\r
    ...
    float maxY = maxR; // 简化包围盒为球或大圆柱\r
    float r = length(pos);\r
    // 如果当前点在包围盒内，或者射线方向延伸 probeDist 后能碰到包围盒\r
    // 这里做个最简单的剔除：如果离原点太远且向外射，则忽略\r
...
\r
    ...
    {\r
        // 在 Log 空间内，从 1e-2 到 1.0 进行平滑过渡 (0.0 -> 1.0)\r
        // 超过 1.0 则保持最大强度\r
...
\r
    ...
    \r
    // 计算内边缘处的基准角速度 (用于确定噪声刷新的物理节奏)\r
    // M=0.5 -> Rs=1.0. 在 Rg 空间计算.\r
    float ReferenceOmega = GetKeplerianAngularVelocity(6.0, 1.0, PhysicalSpinA, PhysicalQ);\r
    \r
    // 设定一个合理的物理周期，内圈每旋转一定圈数，噪声完成一次淡入淡出循环。\r
...
\r
    // 引入垂直漂移：让噪声采样点随时间在Y轴移动，消除单调重复感\r
...
\r
    ...
    {\r
        ...
        {\r
            ...
            \r
            float cloud = max(0.0, valCombined - 0.3-0.7*HAZE_DENSITY_THRESHOLD); // 喷流的heat haze相比吸积盘需要更多空隙，不然看着怪\r
...
\r
        ...
        {\r
            // 情况 A：下一个采样点超出了当前的 RK4 步长范围\r
            // 我们走完这段剩余距离，但不进行采样\r
            // 并更新相位，表示我们已经走了一部分路程\r
            \r
            float PhaseProgress = DistRemainingInRK4 / StepSize;\r
            RayMarchPhase -= PhaseProgress; // 消耗相位\r
            \r
            // 确保相位数值稳定\r
            if(RayMarchPhase < 0.0) RayMarchPhase = 0.0; // 理论不应发生，除非精度误差\r
            \r
            TraveledDist = TotalDist; // 结束本段积分\r
            break;\r
        }\r
\r
...
\r
        // 薄盘优化\r
        vec3 PreviousPos = CurrentPos; // 这一步的起点\r
        //if(PreviousPos.y * SamplePos.y < 0.0)\r
        //{\r
        //\r
        //    vec3 CPoint=(-SamplePos*PreviousPos.y+PreviousPos*SamplePos.y)/(SamplePos.y-PreviousPos.y);\r
        //    SamplePos=CPoint+min(Thin,length(CPoint-PreviousPos))*DirVec*(-1.0+2.0*RandomStep(10000.0*(SamplePos.zx/OuterRadius), fract(iTime * 1.0 + 0.5)));\r
        //    \r
        //\r
        //}\r
        ...
        \r
        // 外层包围盒取主盘与内侧云的并集\r
        // GeometricThin * 1.5 是主盘噪声的包围盒，InnerCloudBound 是内侧云的包围盒\r
...
\r
             ...
             {\r
                 ...
                 /*\r
                 vec3 FluidVel = AngularVelocity * vec3(SamplePos.z, 0.0, -SamplePos.x);\r
                 vec4 U_fluid_unnorm = vec4(FluidVel, 1.0); \r
                 KerrGeometry geo_sample;\r
                 ComputeGeometryScalars(SamplePos, PhysicalSpinA, PhysicalQ, 1.0, 1.0, geo_sample);\r
                 vec4 U_fluid_lower = LowerIndex(U_fluid_unnorm, geo_sample);\r
                 float norm_sq = dot(U_fluid_unnorm, U_fluid_lower);\r
                 vec4 U_fluid = U_fluid_unnorm * inversesqrt(max(1e-6, abs(norm_sq)));\r
                 float E_emit = -dot(iP_cov, U_fluid);\r
...
struct TraceResult {\r
    vec3  EscapeDir;      // 最终逸出方向 (World Space)\r
    float FreqShift;      // 频移 (E_emit / E_obs)\r
    float Status;         // 0=Stop, 1=Sky, 2=Antiverse,3=不透明体积\r
    vec4  AccumColor;     // 体积光颜色 (吸积盘+喷流)\r
    float CurrentSign;    // 最终宇宙符号\r
};\r
\r
...
\r
    ...
    \r
    // [修改]加一个量，追踪光线路径上的全局最小半径\r
    float GlobalMinGeoR = 10000.0;\r
    \r
    if (DistanceToBlackHole > RaymarchingBoundary) //跳过与包围盒间空隙\r
    {\r
...
\r
        ...
        {\r
            ...
            \r
            // 为了Debug重算一遍参数，与 GetHazeForce 保持一致\r
            float rotSpeedBase = 100.0 * HAZE_ROT_SPEED;\r
            float jetSpeedBase = 50.0 * HAZE_FLOW_SPEED;\r
            \r
            // 参数与 GetHazeForce 同步\r
            float ReferenceOmega = GetKeplerianAngularVelocity(6.0, 1.0, PhysicalSpinA, PhysicalQ);\r
            float AdaptiveFrequency = abs(ReferenceOmega * rotSpeedBase) / (2.0 * kPi * 5.14);\r
            AdaptiveFrequency = max(AdaptiveFrequency, 0.1);\r
...
\r
            ...
            {\r
                ...
                \r
                // 修改初始动量 (协变动量空间部分 P_i)\r
                P_cov.xyz += deflection;\r
                \r
                // 重新归一化以保持能量守恒 (E_conserved = -P_t 保持不变，但 P^u P_u = 0 需要维持)\r
                // 原代码后续循环依赖 P_cov 且会重新计算 E，这里简单微扰动量方向即可。\r
                // 实际上，为了保持光子性质 (null geodesic)，改变 P.xyz 后 P_t (或 E) 可能不再匹配。\r
                // 但由于后续 StepGeodesicRK4 会用 E 重新校准 P，这里只需直接加上扰动。\r
                \r
                // 同步更新 RayDir (用于第一步步进方向)\r
    ...
    #endif\r
    \r
    //ApplyHamiltonianCorrectionFORTEST(P_cov, X, E_conserved, PhysicalSpinA, PhysicalQ, GravityFade, CurrentUniverseSign);//debug\r
    // -------------------------------------------------------------------------\r
    // 初始合法性检查与终结半径\r
    // -------------------------------------------------------------------------\r
    ...
    float Q2 = iQ * iQ * CONST_M * CONST_M; // Q^2\r
    \r
\r
...
\r
    ...
    if(bIsNakedSingularity) MaxStep=450.0;//150.0+300.0/(1.0+10.0*(1.0-iSpin*iSpin-iQ*iQ)*(1.0-iSpin*iSpin-iQ*iQ));\r
    // -------------------------------------------------------------------------\r
    // 主循环\r
    // -------------------------------------------------------------------------\r
    int Count = 0;\r
...
\r
        ...
        { \r
            bShouldContinueMarchRay = false;\r
            bWaitCalBack = false;\r
            //Result = vec4(0.0, 0.3, 0.3, 0.0); \r
            break; //视界判定情况1，直接进入视界判定区\r
        }\r
...
\r
                ...
                {\r
                    ...
                    //Result = vec4(0.3, 0.0, 0.3, 1.0); \r
                    break;//识别剔除奇环附近束缚态光子轨道\r
                }\r
                \r
            }\r
            \r
            if(geo.r > InnerHorizonR && lastR < InnerHorizonR) bIntoInHorizon = true;     //检测穿过内视界 \r
            if(geo.r > EventHorizonR && lastR < EventHorizonR) bIntoOutHorizon = true;    //检测穿过外视界 \r
            \r
            ...
            \r
                ...
                if(bIntoInHorizon) { preCeiling = InnerHorizonR + 0.2; } //处理 射线从相机出发 -> 向外运动 -> 调头 -> 向内运动 -> 撞击内视界 的光\r
                if(bIntoOutHorizon) { preCeiling = EventHorizonR + 0.2; }//处理 射线从相机出发 -> 向外运动 -> 调头 -> 向内运动 -> 撞击外视界 的光\r
                \r
            ...
            \r
                ...
                {\r
                    ...
                    {\r
                        bShouldContinueMarchRay = false;\r
                        bWaitCalBack = false;\r
                        //Result = vec4(0.0, 0., 0.3, 0.0);\r
                        break; //视界判定情况2，对凝结在视界前的光提前剔除\r
                    }\r
                }\r
            }\r
        }\r
        \r
        //对动量和位置及其导数做自适应步长。对电荷做自适应步长（Q贡献r^-2项\r
...
\r
        ...
        { \r
            bShouldContinueMarchRay = false; \r
            bWaitCalBack = false;\r
            //Result = vec4(0.3, 0.3, 0.2, 0.0);  \r
            break; //视界判定情况3，凝结在视界\r
        }\r
\r
        //if (Count == 0)\r
        //{\r
        //    dLambda = RandomStep(FragUv, fract(iTime)); // 光起步步长抖动,但是会让高层光子环变糊，考虑到现在吸积盘的层纹去除逻辑已经挪进体积云噪声内部，建议关着。\r
        //}\r
...
\r
    // 状态位定义:\r
    // 0.0 = Absorbed/Lost (视界/超时，且未被体积光完全遮挡)\r
    // 1.0 = Sky (Universe +1)\r
    // 2.0 = Antiverse (Universe -1)\r
    // 3.0 = Opaque (Result.a > 0.99，体积光完全遮挡，其边界与前三者交界不做检查)\r
\r
    if (Result.a > 0.99) {\r
        // 状态 3：不透明体积\r
        res.Status = 3.0; \r
        ...
        \r
        if (CurrentUniverseSign  > 0.0) res.Status = 1.0; // Sky\r
        else res.Status = 2.0; // Antiverse\r
    } \r
    else {\r
        // 状态 0：该方向无吸积盘和喷流以外任何光\r
        res.Status = 0.0; \r
        res.EscapeDir = vec3(0.0);\r
        res.FreqShift = 0.0;\r
    }\r
...
\r
// =============================================================================\r
// SECTION 7: mainImage (Shadertoy 入口)\r
// =============================================================================\r
\r
...
`,I=`//Horizontal gaussian blur leveraging hardware filtering for fewer texture lookups.\r
\r
vec3 ColorFetch(vec2 coord)\r
{\r
 	return texture(iChannel0, coord).rgb;   \r
...
\r
    fragColor = vec4(color,1.0);\r
}`,B=`//Vertical gaussian blur leveraging hardware filtering for fewer texture lookups.\r
\r
vec3 ColorFetch(vec2 coord)\r
{\r
 	return texture(iChannel0, coord).rgb;   \r
...
{    vec2 uv = fragCoord.xy / iResolution.xy;\r
    ...
    \r
\r
    //Tonemapping and color grading\r
...
`;function F(n){let r=n;r=r.replace(/^\/\/#version\s+\d+.*$/gm,""),r=r.replace(/^#version\s+\d+.*$/gm,""),r=r.replace(/^\/\/#pragma.*$/gm,""),r=r.replace(/^\/\/#extension.*$/gm,""),r=r.replace(/^#pragma.*$/gm,""),r=r.replace(/^#extension.*$/gm,"");const t=r.match(/void\s+mainImage\s*\(\s*out\s+vec4\s+(\w+)\s*,\s*in\s+vec2\s+(\w+)\s*\)\s*\{/i);if(t){const e=t[1],i=`out vec4 fragColor;
void main()
{
